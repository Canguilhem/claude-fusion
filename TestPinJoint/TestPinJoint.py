...
def run(context):
    ...
    try:
        ...
        pinY = trackWidth / 2
        pinZ = socketInradius + minShell  # bottom of socket sits minShell above Z=0
        ...
        # Extra material on floor inner surface for the connector
        # =========================
        ...
        # =========================
        # STEP 2: Split at midpoint (straight cut)
        # =========================
        splitPlaneInput = planes.createInput()
        splitPlaneInput.setByOffset(root.yZConstructionPlane,
                                     adsk.core.ValueInput.createByReal(midX))
        splitPlane = planes.add(splitPlaneInput)
        splitPlane.name = "SplitPlane"

        splitFeats = root.features.splitBodyFeatures
        splitFeats.add(splitFeats.createInput(uBody, splitPlane, True))

        # Identify pieces by min X
        ...
        ui.messageBox(
            "Octagonal Pin & Socket Joint (with boss)\n\n"
